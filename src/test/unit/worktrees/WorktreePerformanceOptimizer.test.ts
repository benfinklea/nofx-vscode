...
describe('WorktreePerformanceOptimizer - 100% Coverage', () => {
    ...
    describe('Parallel Operations', () => {
        ...
        it('should queue operations when at max parallel', async () => {
            const agents = Array.from({ length: 5 }, (_, i) => createMockAgent({ id: `parallel-${i}` }));
        ...
        it('should process queue after operation completes', async () => {
            const agents = Array.from({ length: 4 }, (_, i) => createMockAgent({ id: `queue-${i}` }));
        ...
        it('should handle queue operation failures', async () => {
            ...
            await expect(optimizer.createWorktreeOptimized(agent)).rejects.toThrow('Git failed');
        });
    });
    ...
    describe('Circuit Breaker', () => {
        ...
        it('should open circuit breaker after threshold failures', async () => {
            ...
            await expect(optimizer.createWorktreeOptimized(agent)).rejects.toThrow('temporarily disabled');
        ...
        it('should handle half-open state correctly', async () => {
            ...
            expect(mockLoggingService.info).toHaveBeenCalledWith('Circuit breaker closed - Git operations restored');
        });
    });
    ...
    describe('Remove Operations', () => {
        ...
        it('should queue removal when at max parallel ops', async () => {
            const agents = Array.from({ length: 5 }, (_, i) => createMockAgent({ id: `remove-parallel-${i}` }));
        ...
        it('should handle removal failures gracefully', async () => {
            ...
            await expect(optimizer.removeWorktreeOptimized('remove-fail')).rejects.toThrow('Remove failed');
        });
    ...
    describe('Error Handling', () => {
        ...
        it('should handle git command timeouts', async () => {
            ...
            await expect(optimizer.createWorktreeOptimized(agent)).rejects.toThrow('Timeout');
        });
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle rapid cache invalidation', async () => {
            ...
            const promises = Array.from({ length: 10 }, () => optimizer.getWorktreeOptimized('rapid-cache'));
