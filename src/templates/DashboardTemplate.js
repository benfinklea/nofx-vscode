...
class DashboardTemplate {
    ...
    generateDashboardHTML(state, webviewHost) {
        const scriptUri = webviewHost.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'webview', 'dashboard.js'));
        const styleUri = webviewHost.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'webview', 'dashboard.css'));
...
<body>
    ...
    generateSourceOptions(connections) {
        const sources = new Set(connections.map(conn => conn.name));
        return Array.from(sources).map(source => `<option value="${source}">${source}</option>`).join('');
    }
}
