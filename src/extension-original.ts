...
// PersistenceService removed - duplicate of PersistenceService
import { PersistenceService } from './services/PersistenceService';
    ...
    try {
        ...
        ServiceLocator.register('ConfigurationService', () => {
            try {
                ...
                const eventBus = ServiceLocator.get<IEventEmitter, IEventSubscriber>('EventBus');
        ...
        if (workspaceFolder) {
            ...
            const agentPersistence = new PersistenceService(workspaceFolder.uri.fsPath, loggingService);
            ...
            const persistenceService = new PersistenceService(
                context,
                ServiceLocator.get<ILogger>('LoggingService'),
                ServiceLocator.get<IEventEmitter, IEventSubscriber>('EventBus')
            );
            ServiceLocator.register('PersistenceService', persistenceService);
        }
        ...
        agentManager.setDependencies(
            ServiceLocator.get('AgentLifecycleManager'),
            ServiceLocator.get('TerminalManager'),
            ServiceLocator.get('WorktreeService'),
            ServiceLocator.get<IConfiguration>('ConfigurationService'),
            ServiceLocator.get<INotificationService>('NotificationService'),
        ...
        ServiceLocator.register(
            ...
            () =>
                new UIStateManager(
                    ServiceLocator.get<IEventEmitter, IEventSubscriber>('EventBus'),
                    ServiceLocator.get<ILogger>('LoggingService'),
                    ServiceLocator.get('AgentManager'), // Now implements IAgentReader
                    ServiceLocator.get('TaskQueue') // Now implements ITaskReader
                )
        );
        ...
        ServiceLocator.register('MessageFlowDashboard', () =>
        ...
        const agentProvider = new AgentTreeProvider(
            ServiceLocator.get('UIStateManager'),
            ServiceLocator.get('UIStateManager'),
            ServiceLocator.tryGet<TaskToolBridge>('TaskToolBridge')
        );
        ...
        const agentCommands = new AgentCommands(ServiceLocator);
        ...
        if (!isTestMode) {
            ...
            const eventBus = ServiceLocator.get<IEventEmitter, IEventSubscriber>('EventBus') as IEventEmitter,
                IEventSubscriber;
        ...
        const configService = ServiceLocator.get<IConfiguration>('ConfigurationService');
        ...
        try {
            console.log(
                '[NofX Debug] Extension activated successfully',
                {
