...
class MetricsService {
    constructor(configService, logger, eventBus) {
        this.metrics = [];
        this.timers = new Map();
    ...
    initializeConfiguration() {
        try {
            this.isEnabled = this.configService.get(interfaces_1.METRICS_CONFIG_KEYS.ENABLE_METRICS, false);
            this.outputLevel = this.configService.get(interfaces_1.METRICS_CONFIG_KEYS.METRICS_OUTPUT_LEVEL, 'basic');
            this.retentionHours = this.configService.get(interfaces_1.METRICS_CONFIG_KEYS.METRICS_RETENTION_HOURS, 24);
        }
        catch (error) {
    ...
    incrementCounter(name, tags) {
        if (!this.isEnabled || !this.shouldSampleMetric(name))
            return;
        this.addMetric({
            name,
            type: interfaces_1.MetricType.COUNTER,
            value: 1,
            tags,
            timestamp: new Date()
        });
    ...
    recordDuration(name, duration, tags) {
        ...
        this.addMetric({
            name,
            type: interfaces_1.MetricType.HISTOGRAM,
            value: duration,
            tags,
            timestamp: new Date()
        });
    ...
    setGauge(name, value, tags) {
        ...
        this.addMetric({
            name,
            type: interfaces_1.MetricType.GAUGE,
            value,
            tags,
            timestamp: new Date()
        });
    ...
    addMetric(metric) {
        ...
        else if (this.outputLevel === 'basic' && metric.type === interfaces_1.MetricType.COUNTER) {
    ...
    getMetricsSummary() {
        ...
        const histograms = this.metrics.filter(m => m.type === interfaces_1.MetricType.HISTOGRAM);
    ...
    collectSystemMetrics() {
        if (!this.isEnabled || this.outputLevel === 'none')
            return;
    ...
    shouldSampleMetric(name) {
        ...
        return !highFrequencyMetrics.includes(name) || Math.random() < 0.1;
    }
    getDashboardData() {
        const now = new Date();
    ...
    getTopCounters(metrics) {
        const counterMap = new Map();
        metrics
            .filter(m => m.type === interfaces_1.MetricType.COUNTER)
            .forEach(m => {
    ...
    getAverageDurations(metrics) {
        const durationMap = new Map();
        metrics
            .filter(m => m.type === interfaces_1.MetricType.HISTOGRAM)
            .forEach(m => {
